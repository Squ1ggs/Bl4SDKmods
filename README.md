...
| **vehicle_movement** | Vehicle tuning + spawn (`vehicle_move_*`). Optional **BLImGui** panel ? bind **Open Vehicle Movement tab** (Ctrl+Alt+F8). |
| **challenge_ticker** | UVHM ranks + challenge completion. **BLImGui** tab (Ctrl+F7). GPL-3.0. |

Mods menu + console still work without BLImGui. For the panels, install [BLImGui](https://github.com/juso40/blimgui) separately and set the keybind under **Mods ? Keybinds**.

---

## Install (`.sdkmod`)

1. From this repo, take the `.sdkmod` inside the mod folder ? e.g. `vehicle_movement/vehicle_movement.sdkmod`, `challenge_ticker/challenge_ticker.sdkmod`.
