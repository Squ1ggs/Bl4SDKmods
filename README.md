...
**What I built** is only the **`squ1ggs_blimgui`** package here: it **registers one extra tab** inside blimgui’s menu (**“Tuning GUI”**) and draws the controls that talk to my three gameplay mods. That’s **UI inside blimgui’s menu**, not a replacement for blimgui and not the ImGui framework itself.

**Install order:** enable **blimgui** first, then enable **Tuning GUI** (`squ1ggs_blimgui`) if you want that tab.
...
| **squ1ggs_blimgui** | **Tuning GUI** — optional **blimgui** tab that drives the three mods above. Needs **blimgui** and at least one of those mods; it has **no sliders by itself**. |
...
3. For the hub tab: install and enable **blimgui**, then enable **`squ1ggs_blimgui`** (shown in the mod list as **Tuning GUI**).
