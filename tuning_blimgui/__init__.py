...
Oak2 loads this package as ``tuning_blimgui`` (folder / ``.sdkmod`` stem). The in-game mod
name is **Tuning GUI**; ``__author__`` remains **Squ1ggs**.
...
SETTINGS_PATH = _SETTINGS_DIR / "tuning_blimgui.json"
_LEGACY_SETTINGS_SOURCES = (
    _SETTINGS_DIR / "squ1ggs_blimgui.json",
    _SETTINGS_DIR / "squ1ggs_blimgui_tuning.json",
)

if not SETTINGS_PATH.exists():
    for _legacy in _LEGACY_SETTINGS_SOURCES:
        if _legacy.exists():
            with contextlib.suppress(OSError):
                shutil.copy2(_legacy, SETTINGS_PATH)
            break
