"""
Squ1ggs — ImGui tuning panel for standalone BL4 mods (player / vehicle / damage).

On disk this package is ``squ1ggs_blimgui``. Requires the separate **blimgui** mod so ImGui loads.
Each sub-panel appears only if the matching mod is installed (``bl4_player_movement``, ``bl4_vehicle_movement``,
``bl4_damage_and_more``).
"""
...
__version__ = "1.0.2"
__author__ = "Squ1ggs"
MOD_NAME = "Squ1ggs · Tuning"
LOG_PREFIX = "[SQGUI]"
TAB_TITLE = "Squ1ggs · Tuning"
...
def _draw_tuning_tab() -> None:
    ...
    imgui.text_wrapped(
        f"Squ1ggs tuning — {n} companion mod(s) loaded. "
    ...
    if n == 0:
        imgui.text_wrapped(
            "No Squ1ggs tuning mods detected. Install one or more of: "
        ...
        if imgui.begin_tab_bar("Squ1ggsTuningInner##sq"):
...
@command("sqgui_open", description="Open the BL4 Mod Menu on the Squ1ggs · Tuning tab (requires blimgui).")
...
KEY_OPEN_MENU = keybind(
    ...
    display_name="Open BL4 Mod Menu (Squ1ggs · Tuning)",
    description=(
        "Opens the ImGui mod menu (same key as blimgui's default) and ensures the "
        "Squ1ggs · Tuning tab is registered. Rebind in Mods → Keybinds if it conflicts."
    ),
)
...
_squ1ggs_gui_mod = build_mod(
    name=MOD_NAME,
    author=__author__,
    description="ImGui tab for Squ1ggs player / vehicle / damage tuning mods (requires blimgui).",
    version=__version__,
