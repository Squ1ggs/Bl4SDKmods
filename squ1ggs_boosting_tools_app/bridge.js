...
async function ensureBridgePort(force = false) {
  ...
  // 750ms: ULM dump / heavy spawn can stall GIL so 350ms probes false-negative
  // and the EXE refuses catalog loads ("Could not reach the game bridge").
  const hits = await Promise.all(
    order.map((port) => probePort(port, 750).then((data) => ({ port, data })))
  );
...
async function fetchJson(path, options = {}) {
  ...
  // Catalog calls use requireLive:false after dump hitch — prefer advertised port
  // even when the short /status probe missed.
  if (!discover.found) {
    const hinted = advertisedPort();
    if (hinted > 0) activePort = hinted;
  }
  const controller = new AbortController();
...
async function fetchCatalog(name, payload = {}) {
  ...
  // Mob list is larger; dump hitch can also delay first byte — give it room.
  const timeoutMs =
    name === "item_pools" || name === "mob_actors" || name === "io_spawns" ? 20000 : 8000;
  let httpStatus;
  let data;
  try {
    // requireLive:false — do not treat a short /status probe miss (ULM dump) as
    // offline; still hit the advertised port with the full catalog timeout.
    ({ httpStatus, data } = await fetchJson(path, {
      method: "GET",
      timeoutMs,
      requireLive: false,
      rediscover: true,
    ...
    if (error?.name === "AbortError" || /aborted/i.test(text)) {
      throw new Error(
        "Catalog timed out (dump / hitch?) — wait until dump finishes, Refresh status, then Retry.",
      );
    }
    if (/fetch failed|econnrefused|network|ECONNREFUSED/i.test(text)) {
      throw new Error(
        "Could not reach the game bridge — finish any ULM dump, Refresh status, then Retry.",
      );
    }
    throw error;
  }
