...
def _read_header_numbers(serial):
    ...
    Catalog / in-game @U codes can fail a full part parse while still carrying a
    valid ``type, flags, flags, level|`` header. Level override only needs that.
    """
...
def rewrite_item_level(serial, level):
    ...
    Same approach as MSBT: does **not** edit backpack UObjects live — rewrite the
    @U string, then re-deliver. Survives codes that fail ``serial_to_human``.
    """
