"""Player movement adjustment helpers for Squ1ggs Boosting Tools.
...
def _local_player_controller() -> Any | None:
    try:
        return get_pc()
    except Exception:
        return None
...
def _local_player_pawn() -> Any | None:
    pc = _local_player_controller()
    if pc is None:
        return None
...
def _is_local_player_controller(pc: Any) -> bool:
    local = _local_player_controller()
    if local is None or pc is None:
        return False
    try:
        return pc is local or id(pc) == id(local)
    except Exception:
        return False


def _is_local_player_pawn(pawn: Any) -> bool:
    local = _local_player_pawn()
    if local is None or pawn is None:
        return False
    try:
        return pawn is local or id(pawn) == id(local)
    except Exception:
        return False
...
def _write_jump_goal_def_instance(defptr: Any, goal_height: float, initial_z_velocity: float) -> int:
    """Mutate a live FGbxDefPtr JumpGoalDef's resolved instance.

    BL4 resolves the active jump from CurrentJump.JumpGoal, which is an
    FGbxDefPtr.  The useful fields live on defptr.instance, not on the movement
    component as plain floats.  This intentionally changes only the resolved
    runtime instance fields used by the active jump goal.
    """
    if defptr is None:
        return 0
    wrote = 0
    ...
    for field, value in (
        ...
        try:
            ...
            pass
    # Force the JumpGoalDef to actually consume both values.  Your probe showed
    # JumpGoal_Default has bUseGoalHeight=True and bUseInitialZVelocity=False;
    # for modded high jumps we want both to be active.
    for field, value in (
        ("bUseGoalHeight", True),
        ("bUseInitialZVelocity", True),
        ("bClearGravityScaleAtApex", False),
    ):
        try:
            setattr(inst, field, bool(value))
            wrote += 1
        except Exception:
            pass
    return wrote
...
def _restore_jump_goal_def_flags(defptr: Any) -> int:
    """Undo mod JumpGoalDef flags without rewriting heights (remote moon-jump fix)."""
    if defptr is None:
        return 0
    wrote = 0
    try:
    ...
    if inst is None:
        return 0
    for field, value in (
        ("bUseInitialZVelocity", False),
        ("bClearGravityScaleAtApex", True),
    ):
        try:
            setattr(inst, field, bool(value))
            wrote += 1
...
def jump_goal_flags_poisoned(obj: Any) -> bool:
    """True when a prior mobility apply left mod jump flags on this movement object."""
    try:
    ...
    if inst is None:
        return False
    try:
        if bool(getattr(inst, "bUseInitialZVelocity", False)):
            return True
    except Exception:
        pass
    try:
        if not bool(getattr(inst, "bClearGravityScaleAtApex", True)):
            return True
    except Exception:
        pass
    return False
...
def restore_vanilla_jump_goal_flags_on_obj(obj: Any) -> int:
    """Reset JumpGoalDef flags to vanilla without touching GoalHeight / JumpZ."""
    if obj is None or not _uobject_addr(obj):
        return 0
    wrote = 0
    try:
    ...
    if current_jump is not None:
        try:
            wrote += _restore_jump_goal_def_flags(getattr(current_jump, "JumpGoal", None))
        except Exception:
            pass
    try:
        ...
        if callable(get_type) and callable(get_goal):
            jt = get_type()
            try:
                wrote += _restore_jump_goal_def_flags(get_goal(jt))
            except Exception:
                pass
    except Exception:
        pass
...
def _apply_jump_to_obj(obj: Any, jump_goal: float, jump_velocity: float, sprint_jump_goal: float | None = None, jump_hold_time: float | None = None, double_jump_goal: float | None = None) -> int:
    changed = 0
    sprint_goal = float(sprint_jump_goal if sprint_jump_goal is not None else jump_goal)
    double_goal = float(double_jump_goal if double_jump_goal is not None else sprint_goal)
    # Prefer the real BL4 path first: CurrentJump.JumpGoal is an FGbxDefPtr
    # whose resolved instance carries GoalHeight / InitialZVelocity.
    changed += _write_current_jump_goal_def(obj, jump_goal, jump_velocity, sprint_goal, double_goal)
    # Keep old broad reflection as fallback for builds exposing plain fields.
    if _set_jump_goal_field(obj, jump_goal):
        changed += 1
    for container_name in (
        "CurrentJump",
        "DefaultJump",
        ...
        try:
            ...
            container = None
        changed += _write_jump_detail_struct(container, jump_goal, jump_velocity, sprint_goal)
    for meth in (
        "SetJumpGoal",
        "ServerSetJumpGoal",
        "SetJumpZVelocity",
        ...
        val = sprint_goal if "Sprint" in meth else (jump_goal if "Goal" in meth or "Height" in meth else jump_velocity)
        if _call_setter(obj, meth, val):
            changed += 1
    for attr, value in (
        ("JumpZVelocity", jump_velocity),
        ("JumpVelocity", jump_velocity),
        ("JumpImpulse", jump_velocity),
        ("JumpStrength", jump_velocity),
        ("JumpHeight", jump_goal),
        ("JumpGoal", jump_goal),
        ("SprintJumpGoal", sprint_goal),
        ("SprintJumpHeight", sprint_goal),
        ("SprintJumpZVelocity", jump_velocity),
        ("SprintJumpVelocity", jump_velocity),
    ):
    ...
    if jump_hold_time is not None:
        for attr in ("JumpMaxHoldTime", "JumpHoldTime", "MaxJumpHoldTime"):
            if _set_attr(obj, attr, max(0.0, float(jump_hold_time))):
                changed += 1
    # The class exposes SetCurrentJumpType/GetJumpGoalForJumpType; try switching
    # the active jump type through a few harmless enum-like values after writes.
    for val in (0, 1, 2):
        try:
            fn = getattr(obj, "SetCurrentJumpType", None)
            if callable(fn):
                fn(val)
                changed += 1
                break
            ...
            pass
    changed += _refresh_jump_runtime_state(obj)
    return changed
...
def _apply_advanced_to_obj(
    ...
    *,
    ...
    for attr, value in (
        ...
        ("GlidingAcceleration", max(glide_speed * 2.0, 5000.0)),
        ("GlidingDeceleration", max(glide_speed * 0.6, 1600.0)),
    ...
    if _set_int_attr(obj, "JumpMaxCount", jump_count):
        changed += 1
    for meth, value in (
        ("SetWalkableFloorAngle", walkable_floor_angle),
        ("SetWalkableFloorZ", walkable_floor_z),
        ("SetJumpMaxCount", jump_count),
    ):
        try:
            fn = getattr(obj, meth, None)
            if callable(fn):
...
def apply_movement_advanced_to_all_players(
    ...
    *,
    ...
    if local_only or remote_only:
        try:
            from .mobility_runtime import _is_local_party_index, live_party_contexts
        except Exception:
            live_party_contexts = None  # type: ignore[assignment,misc]
        if live_party_contexts is not None:
            contexts = live_party_contexts()
            writes = 0
            touched = 0
            scope = "remote" if remote_only else "local"
            for idx, _name, pc, pawn, move in contexts:
                if remote_only and _is_local_party_index(int(idx)):
                    continue
                if local_only and not _is_local_party_index(int(idx)):
                    continue
                if pc is not None:
                    c = _apply_speed_to_obj(pc, speed_scale, walk_speed) + _apply_jump_to_obj(
                        pc, jump_goal, jump_velocity, sprint_jump_goal, jump_hold_time
                    )
                    c += _apply_advanced_to_obj(
                        pc,
                        gravity_scale=gravity_scale,
                        max_step_height=max_step_height,
                        jump_count=jump_count,
                        jump_off_z_factor=jump_off_z_factor,
                ...
                if pawn is not None:
                    ...
                    for obj in _movement_objects_for_pawn(pawn):
                        pawn_writes += _apply_speed_to_obj(obj, speed_scale, walk_speed)
                        pawn_writes += _apply_jump_to_obj(
                            obj, jump_goal, jump_velocity, sprint_jump_goal, jump_hold_time
                        )
                        pawn_writes += _apply_advanced_to_obj(
                            obj,
                            gravity_scale=gravity_scale,
                            max_step_height=max_step_height,
                            jump_count=jump_count,
                    ...
                    if pawn_writes:
                        ...
                        writes += pawn_writes
            msg = (
                f"Applied movement to {touched} {scope} party member(s): "
                f"speed {speed_scale:.2f}x, walk {walk_speed:.0f}, JumpGoal {jump_goal:.0f}, "
                f"JumpZ {jump_velocity:.0f}, jump count {jump_count}, gravity {gravity_scale:.2f}, "
                f"step {max_step_height:.0f}, floor angle {walkable_floor_angle:.1f}, "
                f"glide {glide_speed:.0f}/{glide_boost:.0f}, "
                f"vault cost {'unchanged' if vault_cost is None else vault_cost}. Writes: {writes}."
            )
            _log(msg)
            return msg
    ...
    touched = 0
    for pc in controllers:
        if local_only and not _is_local_player_controller(pc):
            continue
        if remote_only and _is_local_player_controller(pc):
            continue
        c = _apply_speed_to_obj(pc, speed_scale, walk_speed) + _apply_jump_to_obj(pc, jump_goal, jump_velocity, sprint_jump_goal, jump_hold_time)
        c += _apply_advanced_to_obj(pc, gravity_scale=gravity_scale, max_step_height=max_step_height, jump_count=jump_count, jump_off_z_factor=jump_off_z_factor, walkable_floor_angle=walkable_floor_angle, walkable_floor_z=walkable_floor_z, glide_speed=glide_speed, glide_boost=glide_boost, glide_air_control=glide_air_control, dash_speed=dash_speed, vault_cost=vault_cost)
        if c:
            touched += 1; writes += c
    for pawn in pawns:
        if local_only and not _is_local_player_pawn(pawn):
            continue
        if remote_only and _is_local_player_pawn(pawn):
            continue
        pawn_writes = 0
        for obj in _movement_objects_for_pawn(pawn):
            pawn_writes += _apply_speed_to_obj(obj, speed_scale, walk_speed)
            pawn_writes += _apply_jump_to_obj(obj, jump_goal, jump_velocity, sprint_jump_goal, jump_hold_time)
...
def reset_movement_advanced_all_players(*, local_only: bool = False, remote_only: bool = False) -> str:
...
def set_time_dilation(value: float) -> str:
    # Allow 0.0 for a true pause. Clamping to 0.01 only *slows* the game.
    value = max(0.0, min(64.0, float(value)))
    try:
        ws = ENGINE.GameViewport.World.PersistentLevel.WorldSettings
        ws.TimeDilation = value
        if value <= 0.0:
            return "Game paused (TimeDilation=0)."
...
def _ensure_cheat_manager(pc: Any) -> Any | None:
    ...
    try:
        ...
        if cheat_class is not None:
            cm = unrealsdk.construct_object(cheat_class, pc, "OakCheatManager_Squ1ggsBoost")
            pc.CheatManager = cm
            return cm
    except Exception:
        pass
...
def set_no_target(enabled: bool) -> str:
    try:
        ...
        lib.LockTargetableByAI(get_pc(), "sqbt_no_target", bool(enabled), bool(enabled))
...
def set_noclip(enabled: bool) -> str:
    try:
        ...
        return apply_noclip_pawn(pawn, move, enabled=enabled)
    except Exception as exc:
        return f"Noclip failed: {exc!r}"


def apply_noclip_pawn(pawn: Any, move: Any | None, *, enabled: bool) -> str:
    """Collision off + flying movement (Mobility noclip — not FAAFO fall-through)."""
    moves = _movement_objects_for_pawn(pawn) if pawn is not None else []
    if move is not None and move not in moves:
        moves.insert(0, move)
    if pawn is None or not moves:
        return "Noclip failed: no pawn/movement component found."
    if enabled:
        try:
            ...
            pass
        _set_pawn_collision_enabled(pawn, False)
        for comp in moves:
            try:
                comp.SetMovementMode(5, 0)  # MOVE_Flying
            except Exception:
                pass
        return "Noclip On."
    _set_pawn_collision_enabled(pawn, True)
    for comp in moves:
        try:
            comp.SetMovementMode(1, 0)  # MOVE_Walking
        except Exception:
            pass
    try:
...
def _collision_components_for_pawn(pawn: Any) -> list[Any]:
    comps: list[Any] = []
    try:
        root = getattr(pawn, "RootComponent", None)
        if root is not None:
            comps.append(root)
    except Exception:
        pass
    for meth in ("K2_GetComponentsByClass", "GetComponentsByClass"):
        try:
            ...
            for cls_name in (
                ...
                try:
                    ...
                    batch = fn(cls)
                    if batch:
                        comps.extend(list(batch))
                except Exception:
                    pass
        except Exception:
            pass
    for cls_name in ("CapsuleComponent", "SkeletalMeshComponent"):
        try:
            fn = getattr(pawn, "GetComponentByClass", None)
            if not callable(fn):
                continue
            cls = unrealsdk.find_class(cls_name)
            if cls is None:
            ...
            pass
    return _unique_live_objects(comps)


def _set_pawn_collision_enabled(pawn: Any, enabled: bool) -> None:
    flag = bool(enabled)
    try:
        fn = getattr(pawn, "SetActorEnableCollision", None)
        if callable(fn):
            fn(flag)
    except Exception:
        pass
    try:
        setattr(pawn, "bActorEnableCollision", flag)
    except Exception:
        pass
    for comp in _collision_components_for_pawn(pawn):
        try:
            ...
            pass
        try:
            fn = getattr(comp, "SetGenerateOverlapEvents", None)
            if callable(fn):
                fn(flag)
        except Exception:
            pass
...
def apply_fall_through_pawn(pawn: Any, move: Any | None, *, enabled: bool) -> str:
    """Collision off with gravity / falling — fall through floors (FAAFO)."""
    moves = _movement_objects_for_pawn(pawn) if pawn is not None else []
    if move is not None and move not in moves:
        moves.insert(0, move)
    if pawn is None or not moves:
        return "Fall through map failed: no pawn/movement component found."
    if enabled:
        try:
        ...
        for comp in moves:
            if comp is pawn:
                continue
            try:
                setattr(comp, "bCheatFlying", False)
            except Exception:
                pass
            for attr, value in (
                ("GravityScale", 1.0),
                ...
                try:
                    ...
                    pass
            try:
                comp.SetMovementMode(3, 0)  # MOVE_Falling — gravity, no floor stick
            except Exception:
                try:
                    comp.SetMovementMode(1, 0)
                except Exception:
                    pass
        return "Fall through map ON (collision off, gravity on)."
    _set_pawn_collision_enabled(pawn, True)
    try:
        setattr(pawn, "bCheatFlying", False)
    except Exception:
    ...
    for comp in moves:
        ...
        try:
            ...
            pass
    return "Fall through map OFF."


def set_fall_through_map(enabled: bool) -> str:
    """Legacy host-only entry — prefer apply_fall_through_pawn on a boost target."""
    try:
        pc = get_pc()
        pawn = pawn_for_controller(pc) or getattr(pc, "OakCharacter", None) or getattr(pc, "Pawn", None)
        move = None
        ...
        if pawn is None or move is None:
            return "Fall through map failed: no pawn/movement component found."
        return apply_fall_through_pawn(pawn, move, enabled=enabled)
    except Exception as exc:
        return f"Fall through map failed: {exc!r}"
